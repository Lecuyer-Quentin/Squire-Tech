...
import {
    IconJavascript, IconReact, IconNext, IconNode, IconPHP,
    IconPHPmyAdmin, IconMySQL, IconMongoDB, IconTailwindCSS,
    IconSass, IconBootstrap, IconFigma
} from "./icons/tech/svg";
import { fetchFormations } from "../utils/fetch";
import FormationModel from "../Model/Formation";
...
// Fonction pour rendre les technologies
const renderTech = () => {
    return (
...
// Fonction pour rendre les formations
        ...
        <article className="w-full h-full flex justify-center items-center">
            <ul className="flex items-center justify-center flex-wrap w-full gap-4">
                {formations.length > 0 ? (
                    ...
                    <p>Chargement des formations...</p> // Message de chargement
                )}
            </ul>
        </article>
    );
...
// Composant Intro
const Intro = async () => {
    const formations = await renderFormations(); // Attends la récupération des formations

    return (
        <>
            <Title title="Petit coup d'oeil, sur mon parcours" />
            {formations}  {/* Affiche les formations récupérées */}
            {renderTech()}
            {renderVideo()}
        </>
    );
